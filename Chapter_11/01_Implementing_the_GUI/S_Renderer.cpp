...
void S_Renderer::Update(float l_dT)
{
	...
	{
		C_Position* position = entityMgr->GetComponent<C_Position>(entity, Component::Position);
		C_Drawable* drawable = nullptr;
		if (entityMgr->HasComponent(entity, Component::SpriteSheet))
		{
			drawable = entityMgr->GetComponent<C_Drawable>(entity, Component::SpriteSheet);
		}
		else
		{
			continue;
...
{
	EntityManager* entityMgr = m_systemManager->GetEntityManager();
	if (!entityMgr->HasComponent(l_entity, Component::SpriteSheet))
		return;
	C_SpriteSheet* sheet = entityMgr->GetComponent<C_SpriteSheet>(l_entity, Component::SpriteSheet);
...
void S_Renderer::SortDrawables()
{
	...
	std::sort(m_entities.begin(), m_entities.end(),
		[entityMgr](unsigned int l_1, unsigned int l_2)
		{
			auto pos1 = entityMgr->GetComponent<C_Position>(l_1, Component::Position);
			auto pos2 = entityMgr->GetComponent<C_Position>(l_2, Component::Position);
