	...
	const GUI_Style& l_style)
{
	...
	if (m_sliderType == SliderType::Horizontal)
	{
		m_style[m_state].m_size.x = m_owner->GetSize().x;
	}
	else
	{
		m_style[m_state].m_size.y = m_owner->GetSize().y;
	}
}
...
void GUI_Scrollbar::Update(float l_dT)
{
	...
	sf::Vector2f mousePos =	sf::Vector2f(context->m_eventManager->GetMousePosition(
	...
	{
		m_slider.setPosition(m_owner->GetSize().x - m_slider.getSize().x,
