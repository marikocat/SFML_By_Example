...
void S_Collision::Update(float l_dT)
{
	...
	{
		C_Position* position = entityMgr->
			GetComponent<C_Position>(entity, Component::Position);
		C_Collidable* collidable = entityMgr->
...
void S_Collision::CheckOutOfBounds(C_Position* l_pos, C_Collidable* l_col)
{
	...
	{
		l_pos->SetPosition(m_gameMap->GetMapSize().x * TileSize - 1, l_pos->GetPosition().y);
		l_col->SetPosition(l_pos->GetPosition());
	}
	...
	{
		l_pos->SetPosition(l_pos->GetPosition().x, m_gameMap->GetMapSize().y * TileSize - 1);
		l_col->SetPosition(l_pos->GetPosition());
	}
}
...
void S_Collision::EntityCollisions()
{
	...
	for (auto itr = m_entities.begin(); itr != m_entities.end(); ++itr)
	{
		...
		{
			C_Collidable* collidable1 = entityMgr->GetComponent<C_Collidable>(*itr, Component::Collidable);
			C_Collidable* collidable2 = entityMgr->GetComponent<C_Collidable>(*itr2, Component::Collidable);
