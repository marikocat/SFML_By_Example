...
void GUI_Interface::OnClick(const sf::Vector2f& l_mousePos)
{
	...
	else
	{
		...
		{
			if (!element.second->IsInside(l_mousePos))
				continue;
			element.second->OnClick(l_mousePos);
			event.m_element = element.second->m_name.c_str();
			m_guiManager->AddEvent(event);
		}
...
void GUI_Interface::OnRelease()
{
	...
	for (auto& element : m_elements)
	{
		if (element.second->GetState() != GUI_ElementState::Clicked)
			continue;
...
void GUI_Interface::OnTextEntered(const char& l_char)
{
	...
	{
		if (el.second->GetType() != GUI_ElementType::Textfield)
			continue;
		if (el.second->GetState() != GUI_ElementState::Clicked)
			continue;
		if (l_char == 8)
		{
			// Backspace.
			const auto& text = el.second->GetText();
			el.second->SetText(text.substr(0, text.length() - 1));
			return;
		}
		if (l_char < 32 || l_char > 126)
			return;
		std::string text = el.second->GetText();
		text.push_back(l_char);
		el.second->SetText(text);
		return;
	}
}
...
void GUI_Interface::BeginMoving()
{
	...
	m_moveMouseLast = sf::Vector2f(context->m_eventManager->GetMousePosition(context->m_wind->GetRenderWindow()));
}
...
void GUI_Interface::Update(float l_dT)
{
	sf::Vector2f mousePos = sf::Vector2f(
		m_guiManager->GetContext()->m_eventManager->GetMousePosition(
	...
	for (auto& el : m_elements)
	{
		if (el.second->NeedsRedraw())
		{
			if (el.second->IsControl())
			{
				m_controlRedraw = true;
			}
			else
		...
		if (!el.second->IsActive())
			continue;
		el.second->Update(l_dT);
		if (m_beingMoved)
			continue;
		GUI_Event event;
		event.m_interface = m_name.c_str();
		event.m_element = el.second->m_name.c_str();
		event.m_clickCoords.x = mousePos.x;
		event.m_clickCoords.y = mousePos.y;
		if (IsInside(mousePos) && el.second->IsInside(mousePos)
			&& !m_titleBar.getGlobalBounds().contains(mousePos))
		{
			if (el.second->GetState() != GUI_ElementState::Neutral)
				continue;
			el.second->OnHover(mousePos);
			event.m_type = GUI_EventType::Hover;
			m_guiManager->AddEvent(event);
		}
		else if (el.second->GetState() == GUI_ElementState::Focused)
		{
			el.second->OnLeave();
			event.m_type = GUI_EventType::Leave;
			m_guiManager->AddEvent(event);
...
void GUI_Interface::DefocusTextfields()
{
	...
	for (auto& el : m_elements)
	{
		if (el.second->GetType() != GUI_ElementType::Textfield)
			continue;
		el.second->SetState(GUI_ElementState::Neutral);
		event.m_element = el.second->m_name.c_str();
		m_guiManager->AddEvent(event);
	}
}
