...
{
	// Here we need to release only resources allocated on heap because we won't be using this spritesheet anymore, so we don't neet to set m_texture to empty string.
	// This spritesheet is destructed when character gets discarded. And when character is respawned new spritesheet will be constructed and loaded.
