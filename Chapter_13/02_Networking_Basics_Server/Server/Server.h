...
struct ClientInfo
{
	...
	ClientInfo(const sf::IpAddress& l_ip, const PortNumber& l_port,
		...
		m_lastHeartbeat(l_heartbeat), m_heartbeatWaiting(false), m_heartbeatRetry(0), m_ping(0) {}
...
public:
	...
	void Listen();
	void Update(const sf::Time& l_tiem);
	...
	bool HasClient(const const ClientID& l_id);
