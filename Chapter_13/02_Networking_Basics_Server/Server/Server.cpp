...
void Server::Listen()
{
	...
	while (m_running)
	{
		...
		if (!(packet >> p_id))
		{
			std::cout << "Invalid packet received: unable to extract it." << std::endl;
			continue;
		} // Non-conventioanl packet
		...
		if (id == PacketType::Heartbeat)
		{
			...
			for (auto& itr : m_clients)
			{
				...
				itr.second.m_ping = m_serverTime.asMicroseconds() - itr.second.m_heartbeatSent.asMilliseconds();
...
void Server::Update(const sf::Time& l_time)
{
	...
	if (m_serverTime.asMilliseconds() < 0)
	{
		...
		for (auto& itr : m_clients)
		{
			itr.second.m_lastHeartbeat = sf::milliseconds(std::abs(itr.second.m_lastHeartbeat.asMicroseconds() -
...
bool Server::Start()
{
	...
	if (m_incoming.bind((unsigned int)Network::ServerPort) != sf::Socket::Done) { return false; }
	m_outgoing.bind(sf::Socket::AnyPort);
	SetUp();
	std::cout << "Incoming port: " << m_incoming.getLocalPort() 
		<< ". Outgoing port: " << m_outgoing.getLocalPort() << std::endl;
