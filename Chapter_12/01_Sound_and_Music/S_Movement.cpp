...
void S_Movement::Update(float l_dT)
{
	...
	for (auto& entity : m_entities)
	{
		...
		C_Movable* movable = entityMgr->GetComponent<C_Movable>(entity, Component::Movable);
...
{
	Tile* t = nullptr;
	while (!t && l_elevation >= 0 && l_elevation < Sheet::Num_Layers) {
...
void S_Movement::MovementStep(float l_dT, C_Movable* l_movable, C_Position* l_position)
{
	...
	//std::cout << "FrictionX: " << friction.x << " FrictionY: " << friction.y << std::endl;
