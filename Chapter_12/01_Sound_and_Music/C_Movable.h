...
public:
	...
	{
		m_velocity += l_vec;
		//std::cout << "AddVelocity() VelocityX: " << m_velocity.x << " VelocityY: " << m_velocity.y << std::endl;
		// The end result is checked for being higher than the maximum allowed value on each axis.
	...
	void ApplyFriction(const sf::Vector2f& l_vec)
	{
		...
		//std::cout << "ApplyFriction(): VelocityY: " << m_velocity.y << " FrictionY: " << l_vec.y << std::endl;
