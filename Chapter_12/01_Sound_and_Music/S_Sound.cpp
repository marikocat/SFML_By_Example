...
void S_Sound::Update(float l_dT)
{
	...
	{
		C_Position* c_pos = entityMgr->GetComponent<C_Position>(entity, Component::Position);
		sf::Vector2f position = c_pos->GetPosition();
		unsigned int elevation = c_pos->GetElevation();

		bool isListener = entityMgr->HasComponent(entity, Component::SoundListener);
		if (isListener)
		{
		...
		C_SoundEmitter* c_snd = entityMgr->GetComponent<C_SoundEmitter>(entity, Component::SoundEmitter);
...
void S_Sound::Notify(const Message& l_message)
{
	...
	EntityManager* entityMgr = m_systemManager->GetEntityManager();
	bool isListener = entityMgr->HasComponent(l_message.m_receiver, Component::SoundListener);
	...
	case EntityMessage::Frame_Change:
		if (!entityMgr->HasComponent(l_message.m_receiver, Component::SoundEmitter))
		{
			return;
		}
		EntityState state = entityMgr->GetComponent<C_State>
	...
	bool l_useId, bool l_relative, int l_checkFrame)
{
	...
	// Is a sound emitter.
	EntityManager* entityMgr = m_systemManager->GetEntityManager();
	C_SoundEmitter* c_snd = entityMgr->GetComponent<C_SoundEmitter>(l_entity, Component::SoundEmitter);
	...
	C_Position* c_pos = entityMgr->GetComponent<C_Position>(l_entity, Component::Position);
